...
public class WeeklyPanel extends JPanel{
        ...
        eventCreator.addActionListener(e -> {
            if ((eFrame==null || !eFrame.isVisible())) {
                ...
                eFrame.addCloseListener(close->{
                    user.addCalEvent(selectedEvent);
                    updateAll();
                });
            }
        });
    ...
    public void updateData(){
        TreeSet<CalEvent> events = user.getEvents();
        NavigableSet<CalEvent> dayEvents = new TreeSet<>();
        Calendar startTime;
        Calendar endTime;
        Calendar tempTime;
        HalfHour h;
